...
@section('content')
        ...
        <div class="flex mb-3">
            <div class="w-full">
                <button type="button" id="button-add"
                    ...
                    <i class="ri-add-line align-middle me-1"></i> Tambah Cuaca
                </button>
            </div>
        </div>
            ...
            <table id="myTable" class="table-auto min-w-full bg-white shadow-md border-collapse">
                <thead>
                    <tr class="bg-gray-200">
                        ...
                        <th class="px-4 py-2">Nama Cuaca</th>
                ...
                <tbody>
                    @foreach ($dataCuacaAll as $cuaca)
                        <tr>
                            <td class="border px-4 py-2">
                                <div class="flex justify-center">
                                    @if (file_exists(public_path('assets/icon/' . $cuaca->icon)))
                                        <img class="max-w-full h-[100px] rounded-lg"
                                            src="{{ asset('assets/icon/' . $cuaca->icon) }}" alt="{{ $cuaca->icon }}">
                                    @else
                                        <img class="max-w-full h-[100px] rounded-lg"
                                            src="{{ asset('/storage/' . $cuaca->icon) }}" alt="">
                                    @endif
                                </div>
                            </td>
                            <td class="border px-4 py-2 text-center">{{ $cuaca->nama_cuaca }}</td>
                            <td class="border px-4 py-2">
                                {{ $cuaca->deskripsi }}</td>
                            <td class="border px-4 py-2">{{ $cuaca->video_id }}</td>
                            <td class="text-center border px-4 py-2">
                                <button
                                    ...
                                    data-toggle="modal" id="button-update" data-id="{{ $cuaca->id }}">Edit</button>
                                <button
                                    class="bg-red-500 hover:bg-red-600 text-white font-bold tracking-wide py-2 px-4 rounded-md remove-item-btn"
                                    data-toggle="modal" id="button-delete" data-id="{{ $cuaca->id }}">Remove</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <div id="modal-add" class="hidden fixed z-50 inset-0 items-center justify-center bg-gray-800 bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md h-full max-h-[75vh] overflow-y-auto">
            <h2 class="text-xl font-bold mb-4 text-center">Tambahkan Cuaca</h2>
            <form method="POST" action="{{ route('Admin.fitur-cuaca.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    <label for="nama_cuaca" class="block text-md tracking-wide font-medium text-gray-700">Masukkan Nama
                        Cuaca</label>
                    <input type="text" id="nama_cuaca" name="nama_cuaca" required
    ...
    <div id="modal-update" class="hidden fixed z-50 inset-0 items-center justify-center bg-gray-800 bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md h-full max-h-[75vh] overflow-y-auto">
            <h2 class="text-xl font-bold mb-4 text-center">Update Data Cuaca</h2>
            <form id="formUpdateData" method="POST" enctype="multipart/form-data">
                @csrf
                @method('POST')
                <input type="text" name="id_cuaca" id="id_cuaca" class="hidden">
                <div class="mb-4">
                    <label for="nama_cuaca" class="block text-md tracking-wide font-medium text-gray-700">Masukkan Nama
                        Cuaca</label>
                    <input type="text" id="nama_cuaca_u" name="nama_cuaca" required
                        class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
                <div class="mb-4">
                    <label for="deskripsi" class="block text-md tracking-wide font-medium text-gray-700">Masukkan
                ...
                <div class="flex justify-end">
                    <button type="button" id="button-cancel-update"
                        class="mr-2 py-2 px-4 bg-gray-500 text-white rounded-md hover:bg-gray-600">Cancel</button>
                    <button id="button-update-cuaca" type="submit"
                        class="py-2 px-4 bg-blue-500 text-white rounded-md hover:bg-blue-600">Submit</button>
                </div>
            </form>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '#button-update', function(e) {
                ...
                $.ajax({
                    url: '/fitur-cuaca/' + id + '/cuaca',
                    type: 'GET',
                    success: function(response) {
                        $('#modal-update').removeClass('hidden');
                        $('#modal-update').addClass('flex');
                        $('#id_cuaca').val(response.result.id);
                        $('#formUpdateData').attr('action',
                            `{{ route('Admin.fitur-cuaca.update', ':id') }}`.replace(':id',
                                response.result.id)
                        );
                        $('#nama_cuaca_u').val(response.result.nama_cuaca);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#formDeleteData').attr('action',
                            `{{ route('Admin.fitur-cuaca.destroy', ':id') }}`.replace(
                                ':id',
                                response.result.id)
                        );
                    }
