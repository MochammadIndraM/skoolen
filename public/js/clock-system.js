...
function Timepicker(isClk,is24H,isLight,hour,minute){
	...
	updPickedTm=function(){
		timeStr.innerText=getTime()
	},
	updClockTime=function(){
		updClockHand()
	},
	updClockHand=function(){
		let sec=second*6;
	...
	create=function(){
		...
		clockFace.style.cssText='position:absolute;left:0px;top:0px';
	...
	setClkTheme=function(){
		...
		ctx.fillStyle=isLight?'#000':'#3aa6b9';
		ctx.beginPath();
		ctx.moveTo(10,0);
		ctx.lineTo(0,90);
		ctx.lineTo(20,90);
		...
		ctx.fillStyle=isLight?'#7f7f7f':'#ff9eaa';
		ctx.beginPath();
		ctx.moveTo(6,0);
		ctx.lineTo(0,110);
		ctx.lineTo(12,110);
	...
	this.getTimeString=function(){return getTime()};
	this.hide=function(){
		if(!isHidden){
			isHidden=!isHidden;
			tpick.style.display='none';
